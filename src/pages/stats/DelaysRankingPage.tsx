import { Box, Chip, Divider, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TableSortLabel, Typography, useMediaQuery, useTheme } from "@mui/material";
...
const METRIC_COLUMNS: { id: Exclude<SortKey, "name">; labelKey: string; render: (metrics: RankingMetrics) => React.ReactNode }[] = [
    { id: "count", labelKey: "allCitiesDelays.vehiclesCount", render: (metrics) => metrics.count },
    {
        id: "avg",
        labelKey: "delays.averageDelay",
        render: (metrics) => <Chip size="small" label={`${metrics.avg} min`} className={`bg-${delayMinutesClass(metrics.avg)} text-white`} sx={{ fontWeight: 500 }} />,
    },
    { id: "median", labelKey: "delays.medianDelay", render: (metrics) => `${metrics.median} min` },
];
...
export default function DelaysRankingPage() {
    ...
    const rows = useMemo(() => {
        ...
        for (const [cityKey, city] of Object.entries(data?.cities ?? {})) {
            ...
            result.push({ cityKey, name: city.name, metrics: metrics ?? { count: 0, avg: 0, median: 0 } });
        }
    ...
    return (
        <>
            <PageHeader title={t("allCitiesDelays.pageTitle")} />
            <div className="page">
                {loading && !data ? (
                                ...
                                <Table size="small" aria-label="delays table">
                                    <TableHead>
                                        <TableRow>
                                            ...
                                            <TableCell>
                                                <TableSortLabel active={orderBy === "name"} direction={orderBy === "name" ? order : "asc"} onClick={() => sortBy("name")}>
                                                    {t("allCitiesDelays.city")}
                                                </TableSortLabel>
                                            </TableCell>
                                            {METRIC_COLUMNS.map((column) => (
                                                <TableCell key={column.id} align="right">
                                                    <TableSortLabel active={orderBy === column.id} direction={orderBy === column.id ? order : "desc"} onClick={() => sortBy(column.id)}>
                                                        {t(column.labelKey)}
                                                    </TableSortLabel>
                                                </TableCell>
                                            ))}
                                    ...
                                    <TableBody>
                                        {rows.map((row, index) => {
                                            ...
                                            return (
                                                <TableRow key={row.cityKey} hover sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                                                    <TableCell>{index + 1}.</TableCell>
                                                    <TableCell component="th" scope="row">
                                                        <Link to={`/${row.cityKey}/opoznienia-autobusy-tramwaje`} style={{ color: "var(--primary)" }}>
                                                            {row.name}
                                                        </Link>
                                                    </TableCell>
                                                    {METRIC_COLUMNS.map((column) => (
